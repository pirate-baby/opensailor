...
class VesselAccessRequest(models.Model):
    ...
    reviewed_at = models.DateTimeField(
        null=True, blank=True, help_text=_("When this request was reviewed")
    )
    ...
    class Meta:
        ...
        unique_together = [
            ["vessel", "requester"]
        ]  # One active request per user per vessel
        indexes = [
    ...
    def __str__(self):
        return (
            f"{self.requester.username} -> {self.vessel.name} ({self.requested_role})"
        )
