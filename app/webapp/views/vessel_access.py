...
class VesselAccessRequestForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'message': forms.Textarea(attrs={'rows': 3, 'placeholder': 'Why do you need access to this vessel?'}),
...
def vessel_access_request(request, pk):
    ...
    existing_request = VesselAccessRequest.objects.filter(
        ...
        status=VesselAccessRequest.Status.PENDING
    ).first()
    
    if existing_request:
        messages.info(request, "You already have a pending access request for this vessel.")
        return redirect('vessel_detail', pk=vessel.pk)
...
def vessel_access_approve(request, pk, request_id):
    ...
    vessel = get_object_or_404(Vessel, pk=pk)
    access_request = get_object_or_404(VesselAccessRequest, pk=request_id, vessel=vessel)
...
def vessel_remove_user(request, pk, user_id):
    ...
    messages.success(request, f"Removed access for {user.get_full_name() or user.username}.")
    return redirect('vessel_manage_roles', pk=vessel.pk)
...
def vessel_add_user(request, pk):
    ...
    messages.success(request, f"Added {user.get_full_name() or user.username} as {role_name}.")
    return redirect('vessel_manage_roles', pk=vessel.pk)
