...
class CSVImportMixin:
    def get_urls(self):
        urls = super().get_urls()
        my_urls = [
            path('import-csv/', self.import_csv, name=f'{self.model._meta.model_name}_import_csv'),
            path('download-template/', self.download_template, name=f'{self.model._meta.model_name}_download_template'),
        ]
        return my_urls + urls
    ...
    def download_template(self, request):
        ...
        if instance:
            ...
            values = [getattr(instance, f.name) for f in self.model._meta.fields if f.name != 'id']
        else:
...
class SailboatAdmin(CSVImportMixin, admin.ModelAdmin):
    ...
    def process_csv(self, reader):
        for row in reader:
            ...
            # Handle designers
            if 'designers' in row:
                designer_names = [name.strip().lower() for name in row['designers'].split(',')]
    ...
    def process_csv(self, reader):
        for row in reader:
            sailboat = Sailboat.objects.get(
                name=row['sailboat_name'].lower(),
                make__name=row['make_name'].lower()
            )
